...
GAME = {
    ...
    # Who's already been offered (and resolved or skipped) their Telepathic
    # Link turn this Inspect phase - enforces Martian Manhunter always
    # going before Miss Martian, and stops a re-invite mid-phase.
    "telepathic_responded_cids": [],
    "liar_decoys": {},
}
...
def public_state(reveal_names):
    ...
    if reveal_names:
        ...
        state["telepathic_responded_cids"] = GAME["telepathic_responded_cids"]
        state["eligible_telepathic_characters"] = eligible_telepathic_characters()
...
def _set_phase_by_index(idx, error_sid=None):
    ...
    if old_phase == "Inspect" and (idx is None or PHASES[idx] != "Inspect"):
        ...
        GAME["telepathic_responded_cids"] = []
...
@socketio.on("start_arrange")
def on_start_arrange():
    """Host clicks ARRANGE - the manual alternative to letting Shuffle
    randomize seats. Builds a fresh circle of empty, numbered seats (one
    per locked-in player) for drag-and-drop placement, discarding whatever
    seating existed before (same fresh-start semantics as Shuffle)."""
    if not GAME["roster_locked"]:
        return
    players = [p["name"] for p in GAME["players"]]
    if not players:
        return
    GAME["seats"] = ["" for _ in players]
    log_activity(f"Host started manually arranging seats for {len(players)} players")
    broadcast()


@socketio.on("assign_seat")
def on_assign_seat(data):
    """Host drags a player's name from the Unseated Players pool (or from
    another chair, to relocate them) onto an empty numbered chair. Only
    ever fills a genuinely empty seat - dropping on an occupied one is a
    no-op; the host clears it first (clear_seat) if they want to swap."""
    name = (data.get("name") or "").strip()
    try:
        seat_index_val = int(data.get("seat_index"))
    except (TypeError, ValueError):
        return
    if not name:
        return
    if not any(p["name"].strip().lower() == name.strip().lower() for p in GAME["players"]):
        return  # not a real player in the roster
    seats = GAME["seats"]
    if seat_index_val < 0 or seat_index_val >= len(seats):
        return
    if seats[seat_index_val]:
        return  # target chair is already occupied
    # If this player is already seated elsewhere, relocate them rather
    # than leaving a duplicate behind.
    for i, occupant in enumerate(seats):
        if occupant.strip().lower() == name.strip().lower():
            seats[i] = ""
    seats[seat_index_val] = name
    log_activity(f"{name} seated in Chair {seat_index_val + 1}")
    broadcast()


@socketio.on("clear_seat")
def on_clear_seat(data):
    """Host clicks an occupied chair during Arrange to empty it again,
    returning that player to the Unseated Players pool."""
    try:
        seat_index_val = int(data.get("seat_index"))
    except (TypeError, ValueError):
        return
    ...
    if seat_index_val < 0 or seat_index_val >= len(seats):
        return
    if seats[seat_index_val]:
        log_activity(f"{seats[seat_index_val]} removed from Chair {seat_index_val + 1}")
        seats[seat_index_val] = ""
        broadcast()
...
def on_get_my_card(data):
    ...
    socketio.emit("my_card_result", {
        ...
        "telepathic_links": (
            [GAME["characters"][lcid]["player_name"] for lcid in GAME["telepathic_links"].get(cid, [])
             if GAME["characters"].get(lcid, {}).get("player_name")]
            if cid in TELEPATHIC_CHARACTERS else None
        ),
    }, room=request.sid)
...
def eligible_telepathic_link_targets(inspector_cid):
    """Active players not already linked to this inspector, not the
    inspector themselves, and not already Rescued (not in the
    Watchtower - once safe there, they're outside the game's reach)."""
    ...
    return [
        ...
        and not st.get("rescued")
        and st["player_name"].strip().lower() != pname.lower()
    ]


def eligible_telepathic_characters():
    """Active Martian Manhunter/Miss Martian, whose Telepathic Link
    ability is currently visible, in strict invite order - Martian
    Manhunter is always offered before Miss Martian. Powers the host's
    Inspect-phase queue the same way eligible_inspectors()/
    eligible_protectors() do for their own phases."""
    order = ["martian_manhunter", "miss_martian"]
    return [
        {"id": cid, "name": display_name_for(cid)}
        for cid in order
        if GAME["characters"].get(cid, {}).get("active")
        and _visible_phase_abilities(cid, "Inspect")
    ]


def _character_signal(cid):
    """The plain-text physical 'signal' a character's player uses to
    discreetly identify themselves (e.g. Superman - draw an 'S' on your
    chest), with the leading 'SIGNAL - ' / 'SIGNAL:' label stripped."""
    raw = (CARDS.get(cid, {}).get("signal") or "").strip()
    return re.sub(r"^signal\s*[-:]\s*", "", raw, flags=re.IGNORECASE) or "No signal on file for this character."
...
def on_send_telepathic_link_prompt(data):
    ...
    player to Telepathically Link with - only usable during Inspect!,
    one at a time, and Martian Manhunter always goes first if he's
    active and hasn't had his turn yet this phase."""
    cid = data.get("id")
    ...
    if current_phase != "Inspect" or not _visible_phase_abilities(cid, "Inspect"):
        return
    if cid in GAME["telepathic_responded_cids"]:
        return  # already had their Telepathic Link turn this phase
    if GAME["active_telepathy_cid"] is not None:
        return  # someone else is already mid-pick
    if cid == "miss_martian":
        mm_st = GAME["characters"].get("martian_manhunter")
        mm_eligible = bool(mm_st) and mm_st["active"] and _visible_phase_abilities("martian_manhunter", "Inspect")
        if mm_eligible and "martian_manhunter" not in GAME["telepathic_responded_cids"]:
            return  # Martian Manhunter must be offered (and resolved/skipped) first
...
@socketio.on("skip_telepathic")
def on_skip_telepathic(data):
    """Host marks Martian Manhunter (or Miss Martian) as done with
    Telepathic Link for this phase without them linking anyone - mainly
    so Miss Martian isn't stuck waiting behind a Martian Manhunter who
    doesn't want to use the ability this round."""
    cid = data.get("id")
    if cid not in TELEPATHIC_CHARACTERS:
        return
    if GAME["active_telepathy_cid"] == cid:
        GAME["active_telepathy_cid"] = None
    if cid not in GAME["telepathic_responded_cids"]:
        GAME["telepathic_responded_cids"].append(cid)
        log_activity(f"{display_name_for(cid)} was skipped for Telepathic Link this phase")
        broadcast()
...
def on_submit_telepathic_link_target(data):
    ...
    the table - adds them to that Martian's growing network (a running
    list the Martian can review anytime from their Links tab). No
    identities change hands yet: the target just privately learns their
    own Signal, the discreet physical tell they should use with their
    new Martian teammate from now on. Identities only surface later, all
    at once, if the Martian activates Telepathic Team."""
    ...
    if not target_cid or not GAME["characters"][target_cid]["active"]:
        return
    if GAME["characters"][target_cid].get("rescued"):
        return  # already in the Watchtower - not in Watchtower's reach

    GAME["active_telepathy_cid"] = None
    if inspector_cid not in GAME["telepathic_responded_cids"]:
        GAME["telepathic_responded_cids"].append(inspector_cid)
    links = GAME["telepathic_links"].setdefault(inspector_cid, [])
    if target_cid not in links:
        links.append(target_cid)
    log_activity(f"{display_name_for(inspector_cid)} Telepathically Linked with {target_name}")

    target_sid = _sid_for_player(target_name)
    if target_sid:
        socketio.emit("telepathic_link_alert", {
            "signal": _character_signal(target_cid),
        }, room=target_sid)
    broadcast()
